...
if [ -e 'Makefile.PL' ]; then
    if ! command -v cpanm 2>&1 >/dev/null; then
        ...
        # echo "Installing dependencies via CPANMinus to... ~/perl5/bin";
        # PATH=~/perl5/bin cpanm --installdeps .
    fi;

    echo "Prepare necessary perl modules"
    cpanm --sudo inc::Module::Install
    cpanm --sudo Module::Install::Debian
    ...
    *Module::Install::Debian::_dpkg_install = sub {
    ...
    #     # TODO, enforce versions!
        ...
        for dep in ${PERL_DEPS}; do
            ...
            if [ $DEB_DEP_CANDS ]; then
                echo " - hit: $DEB_DEP_CANDS"
            else
                echo " - miss: $dep"
            fi;
        ...
        #     cpanm --sudo $dep
        # done
        echo "Install missing packages directly via CPAN: $PERL_DEPS"
        cpanm --sudo $PERL_DEPS
        # echo "Installing dependencies via CPANMinus"
        # PATH=~/perl5/bin cpanm --sudo --installdeps .
